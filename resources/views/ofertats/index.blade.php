...
@section('title', ' | Ofertas Emprego Index')
    ...
    <div class="container mb-4">
        <nav class="navbar navbar-light my-4">
            <div class="container-fluid">
                <h3 class="text-primary w-25"><b>OFERTAS EMPREGO</b></h3>


                <form class="d-flex" action="{{ route('ofertats.index') }}" method="GET">
                    <!-- BOTÓN ENGADIR NOVO -->
                    <div class="mr-4">
                        <a href="{{ route('ofertats.create') }}" class="btn btn-white mb-3" alt="Engadir nova oferta">
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    @else
                        ...
                        @foreach ($ofertats as $ofertat)
                            <tr>
                                <td>{{ $ofertat->nome }}</td>
                                <td>{{ $ofertat->datadende }}</td>
                                <td>{{ $ofertat->dataata }}</td>
                                <td>{{ $ofertat->lugar }}</td>
                                <td>{{ $ofertat->tipo }}</td>
                                <td>{{ $ofertat->numhoras }}</td>
                                <td>{{ $ofertat->numprazas }}</td>
                                <!-- Iconos tabla -->
                                <td class="d-flex">
                                    <a href="{{ route('ofertats.show', $ofertat->id) }}">
                                    ...
                                    <a href="{{ route('ofertats.edit', $ofertat->id) }}">
                                    ...
                                    @endif
                                    <form action="{{ route('ofertats.destroy', $ofertat->id) }}" method="POST">
                                        @csrf
                                        {{ method_field('DELETE') }}
                                        {{-- <input type="submit" value="Eliminar" onclick="return EliminarRegistro('Eliminar ofertat')"> --}}
                                        <button class="delete border-0 btn-link"
                                            onclick="return EliminarRegistro('Eliminar ofertat')">
